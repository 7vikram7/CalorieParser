...
| AI | Google Gemini (`gemini-flash-latest`) + Groq (`openai/gpt-oss-120b`) + LangGraph | called from FastAPI | Server-side only — keys must never reach the browser. Provider clients + model constants live in `backend/app/core/clients.py` (single source of truth) so `foods.py` never imports either SDK directly. **The Groq model has already changed once** — the original choice, `llama-3.3-70b-versatile`, was silently removed from Groq's lineup entirely sometime between 2026-08-14 and 2026-08-18 (no deprecation notice, just gone — confirmed via a live `/v1/models` call and a 404 `model_not_found` in production). Replaced with `openai/gpt-oss-120b` after live-testing it against the same prompts. Lesson: a third-party model ID is not a permanent identifier — check `docs/learning-log.md` Level 12 before assuming any hardcoded model name will keep working indefinitely. **Gemini's free tier is capped at `GenerateRequestsPerDayPerProjectPerModel-FreeTier` = 20 requests/day** for `gemini-3.7-flash` (found via a real 429) — shared across local dev and production since they use the same key. Simple descriptions go straight to Groq (no meaningful daily cap, sub-second responses). Complex/multi-item descriptions go through a LangGraph agent pipeline as of Phase 3d (2026-08-14, `backend/app/core/agents.py`): parse into items (Groq) → look each up in USDA (no LLM) → combine into an estimate (Groq) → rules-based validation, retrying the estimate step with feedback up to twice. If that pipeline fails for any reason, `backend/app/core/llm.py`'s `estimate_grounded()` falls back to the older single-shot Gemini tool-calling flow, and if that ALSO fails, falls back once more to the plain Groq estimate — this endpoint should never 502. An `estimate_cache` table (SHA256-hash of the description) is checked before any of this runs, so repeat descriptions cost zero LLM calls. |
...
- `POST /v1/foods/estimate` now calls Gemini and/or Groq instead of the
  ...
  Phase 3a): simple descriptions go to Groq (`openai/gpt-oss-120b` as of
  2026-08-18 — see the AI row above for why the model changed),
  and an `estimate_cache` table short-circuits repeat descriptions before
  any provider is called. As of Phase 3d (2026-08-14): complex/multi-item
  descriptions now go through a LangGraph agent pipeline
  (`backend/app/core/agents.py`) instead of a single Gemini call — see the
  AI row in the stack table above and `backend/app/core/llm.py`. Verified
  working end-to-end against real production data, including forcing a
